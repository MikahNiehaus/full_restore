...
class Learner():
    ...
    def load(self, file:PathLikeOrBinaryStream=None, device:torch.device=None, strict:bool=True,
        ...
        state = torch.load(source, map_location=device, weights_only=False)  # Explicitly set weights_only to False for PyTorch 2.6+ compatibility
    ...
    def purge(self, clear_opt:bool=True):
        ...
        gc.collect()
        state = torch.load(tmp_file, weights_only=False)  # Explicitly set weights_only to False for PyTorch 2.6+ compatibility
        os.remove(tmp_file)
