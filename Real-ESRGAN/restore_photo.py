...
try:
    ...
    patch_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "realesrgan_patch.py")
    ...
    print("[INFO] Applied Real-ESRGAN compatibility patch in restore_photo.py")
...
def restore_old_photo(input_path, output_dir=None, scale=8):
    ...
    try:
        ...
        try:            # Define model path to the downloaded weights
            weights_dir = os.path.join(os.path.dirname(__file__), 'weights')
            model_path = os.path.join(weights_dir, 'realesr-general-x4v3.pth')
            
            if not os.path.exists(model_path):
                raise FileNotFoundError(f"Real-ESRGAN model file not found at: {model_path}")
